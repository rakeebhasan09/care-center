...
const BookingForm = ({ service }) => {
	...
	const totalCost = price * durationValue * multiplier;
				...
				>
					...
					{activeStep === STEPS.DURATION && (
						<div className="card">
							...
							<button
								onClick={() => setActiveStep(STEPS.LOCATION)}
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
								...
								<div>
									...
									>
										...
										{division &&
											districtsByDivision[division]?.map(
												(dist) => (
													<option
														key={dist}
														value={dist}
		...
		${!isLocationFormValid ? "opacity-50 cursor-not-allowed" : ""}`}
							...
							<div className="space-y-6">
								...
								<div className="flex gap-3 p-4 bg-(--secondary)/20 rounded-xl">
									...
									<p className="text-sm text-(--muted-foreground)">
										...
										hours. You&apos;ll receive a
										confirmation email with caregiver
										details.
									</p>
								</div>
							</div>
					...
					{activeStep === STEPS.CONFIRM && (
						<div className="flex flex-wrap gap-5 mt-6">
							...
							<button
								onClick={() => setActiveStep(STEPS.CONFIRM)}
								className="btn btn-primary border border-primary flex flex-1 items-center gap-2"
							>
								Confirm Booking <CircleCheck size={16} />
							</button>
						</div>
					)}
				</form>
